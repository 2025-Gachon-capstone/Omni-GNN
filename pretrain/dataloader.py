...
class InstacartDataloader:
    ...
    def preprocess(self, user2seq):
        ...
        max_len = int(self.args.max_seq_length) - 1  # [USR] 토큰 하나 제외
        overlap = 60 # 100 * 0.6
        
        if max_len <= 0:
            raise ValueError(f"max_seq_length must be >= 2, got {self.args.max_seq_length}")
        if not (1 <= overlap < max_len):
            raise ValueError(f"overlap must be in [1, {max_len-1}], got {overlap}")
        ...
        for user_id, seq in user2seq.items():
            ...
            # 블록 단위로 전진 (블록 크기 = max_len)
            start = 0
            while start < L:
                end = min(start + max_len, L)
                block_len = end - start

                # 1) 메인 블록 추가 (full 또는 tail)
                seqs.append((user_id, seq[start:end]))

                # 2) 블록이 full일 때만, 마지막 60토큰으로 보조 윈도우 추가
                if block_len == max_len:
                    tail_start = end - overlap  # 블록 끝에서 60토큰
                    # tail_start >= start 보장 (overlap < max_len)
                    seqs.append((user_id, seq[tail_start:end]))

                # 다음 블록으로 이동 (비중첩 블록 이동)
                start += max_len
